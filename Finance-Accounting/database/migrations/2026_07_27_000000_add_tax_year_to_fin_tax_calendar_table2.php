...
/**
 * Adds fin_tax_calendar.tax_year and fin_tax_calendar.tax_month.
 *
 ...
 * which silently missed those rolled-over rows. tax_year (and, for
 * monthly VAT calculations, tax_month) stores the actual tax period
 * explicitly so that filtering is correct regardless of which month the
 * deadline falls in.
 */
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('fin_tax_calendar', function (Blueprint $table) {
            ...
            $table->unsignedTinyInteger('tax_month')->nullable()->after('tax_year')->index();
        });
    ...
    {
        Schema::table('fin_tax_calendar', function (Blueprint $table) {
            $table->dropColumn(['tax_year', 'tax_month']);
        });
    }
};
