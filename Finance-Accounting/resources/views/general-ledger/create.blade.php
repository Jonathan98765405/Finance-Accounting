...
@if(!\App\Models\Role::activeRoleCanManageLedger())
    <div class="flex flex-col items-center justify-center py-20 text-center">
        <div class="w-16 h-16 rounded-full bg-red-100 text-brand-red mx-auto flex items-center justify-center mb-4">
            <i data-lucide="shield-alert" class="w-8 h-8"></i>
        </div>
        <h2 class="text-2xl font-bold text-navy mb-2">Access Denied</h2>
        <p class="text-sm text-slate-500 mb-6">You do not have permission to create journal entries.</p>
        <a href="{{ route('ledger.index') }}" class="inline-flex rounded-xl px-5 py-2.5 text-sm font-semibold text-white bg-navy hover:bg-navy-700">Return to General Ledger</a>
    </div>
@else
    @if(session('error'))
