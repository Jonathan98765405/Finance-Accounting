<?php $__env->startSection('page-title', 'Finance & Accounting | Financial Reports'); ?>
<?php $__env->startSection('page-title-heading', 'Financial Reports'); ?>
<?php $__env->startSection('page-subtitle', 'Monitor financial performance and ensure regulatory compliance.'); ?>

<?php $__env->startSection('content'); ?>
    <?php echo $__env->make('financial-reports.header', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
      <div class="bg-white rounded-2xl shadow-card p-5">
        ...
        <p class="text-2xl font-extrabold text-navy mt-1"><?php echo e($fmt($taxSummary['totalDue'])); ?></p>
      </div>
      <div class="bg-white rounded-2xl shadow-card p-5">
        <p class="text-sm font-bold text-brand-green">Filed YTD</p>
        <p class="text-2xl font-extrabold text-navy mt-1"><?php echo e($fmt($taxSummary['filedYtd'])); ?></p>
      </div>
      <div class="bg-white rounded-2xl shadow-card p-5">
        <p class="text-sm font-bold text-slate-500">Pending Filings</p>
        <p class="text-2xl font-extrabold text-navy mt-1"><?php echo e($taxSummary['pendingFilings']); ?></p>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-slate-100">
            <?php $__currentLoopData = $taxCalculation; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tax): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <tr>
                <td class="py-2"><?php echo e($tax['type']); ?></td>
                <td class="py-2"><?php echo e($tax['rate']); ?></td>
                <td class="py-2"><?php echo e($fmt($tax['taxableAmount'])); ?></td>
                <td class="py-2 font-semibold"><?php echo e($fmt($tax['amountDue'])); ?></td>
                <td class="py-2"><?php echo e($tax['deadline']); ?></td>
                <td class="py-2">
                  <button type="button" class="js-tax-status font-semibold <?php echo e($statusColor($tax['status'])); ?> hover:underline"
                    data-type="<?php echo e($tax['type']); ?>" data-rate="<?php echo e($tax['rate']); ?>" data-taxable="<?php echo e($tax['taxableAmount']); ?>"
                    data-amount="<?php echo e($tax['amountDue']); ?>" data-deadline="<?php echo e($tax['deadline']); ?>"
                    data-status="<?php echo e($tax['status']); ?>">
                    <?php echo e($tax['status']); ?>
...
<?php $__env->startPush('scripts'); ?>
    <script>
      ...
      const CF_CURRENT_YEAR = <?php echo e($selectedYear); ?>;
      const CASHFLOW_BY_YEAR = { [CF_CURRENT_YEAR]: <?php echo json_encode($cashflowMonthly, 15, 512) ?> };
      const TAX_SUMMARY_BY_YEAR = { [CF_CURRENT_YEAR]: <?php echo json_encode($taxSummary, 15, 512) ?> };
      const TAX_CALCULATION_BY_YEAR = { [CF_CURRENT_YEAR]: <?php echo json_encode($taxCalculation, 15, 512) ?> };

      async function loadCashflowYear(year) {
        if (CASHFLOW_BY_YEAR[year]) return;

        const res = await fetch(`<?php echo e(url('/financial-reports/cashflow-tax/data')); ?>?year=${year}`, {
      ...
      let TAX_CALENDAR_ITEMS = <?php echo json_encode($taxCalendar, 15, 512) ?>;
      const TAX_CAL_CSRF = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
      const TAX_CAL_STORE_URL = <?php echo json_encode(route('financial-reports.tax-calendar.store'), 15, 512) ?>;
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Finance-Accounting\resources\views/financial-reports/cashflow-tax.blade.php ENDPATH**/ ?>