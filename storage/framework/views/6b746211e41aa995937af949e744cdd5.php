<?php $__env->startSection('page-title', 'All Invoices | Finance & Accounting'); ?>
<?php $__env->startSection('page-title-heading', 'All Invoices'); ?>
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6">
            ...
            <h2 class="text-3xl sm:text-4xl font-extrabold text-navy mt-2"><?php echo e($totalInvoices); ?></h2>
        </div>
        <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6">
            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Paid</p>
            <h2 class="text-3xl sm:text-4xl font-extrabold text-brand-green mt-2"><?php echo e($paidInvoices); ?></h2>
        </div>
        <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6">
            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Unpaid</p>
            <h2 class="text-3xl sm:text-4xl font-extrabold text-brand-orange mt-2"><?php echo e($unpaidInvoices); ?></h2>
        </div>
        <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6">
            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Overdue</p>
            <h2 class="text-3xl sm:text-4xl font-extrabold text-brand-red mt-2"><?php echo e($overdueInvoices); ?></h2>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white rounded-2xl shadow-card border border-slate-200/80 p-4 no-print">
        ...
        <div class="flex flex-wrap items-center gap-3">
            <a href="<?php echo e(route('receivable.dashboard')); ?>"
            ...
            <a href="javascript:void(0)" onclick="handleProtectedNavigation('<?php echo e(route('receivable.invoice')); ?>')"
    ...
    <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6 no-print">
        <form id="filterForm" method="GET" action="<?php echo e(route('receivable.allinvoices')); ?>">
            <div class="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
                <div class="relative flex-1">
                    <i data-lucide="search" class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5"></i>
                    <input type="text" name="search" id="searchInput" value="<?php echo e(request('search')); ?>" placeholder="Search invoice number or customer..." class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy transition">
                </div>
                <div class="flex flex-wrap items-center gap-3">
                    <select name="status" id="statusFilter" class="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy transition min-w-[140px]">
                    ...
                    <button type="submit" class="flex items-center gap-2 bg-navy hover:bg-navy-700 text-white px-6 py-3 rounded-xl font-semibold text-sm transition">
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    <?php $__empty_1 = true; $__currentLoopData = $invoices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/70 transition">
                            <td class="px-6 py-4 font-bold text-navy"><?php echo e($invoice->invoice_number); ?></td>
                            <td class="px-6 py-4 text-slate-600 font-medium"><?php echo e($invoice->customer->customer_name ?? 'No Customer'); ?></td>
                            <td class="px-6 py-4 text-slate-500"><?php echo e(\Carbon\Carbon::parse($invoice->invoice_date)->format('M d, Y')); ?></td>
                            <td class="px-6 py-4 text-slate-500"><?php echo e(\Carbon\Carbon::parse($invoice->due_date)->format('M d, Y')); ?></td>
                            <td class="px-6 py-4 text-right font-bold text-slate-700">&#8369;<?php echo e(number_format($invoice->total, 2)); ?></td>
                            <td class="px-6 py-4 text-center">
                                ...
                                ?>
                                <span class="<?php echo e($badgeClass); ?> px-3 py-1 rounded-full text-xs"><?php echo e($invoice->status); ?></span>
                            </td>
                            <td class="px-6 py-4 text-center no-print">
                                <div class="flex justify-center gap-1.5">
                                    <button type="button" onclick="openInvoiceDetailsModal(<?php echo e($invoice->id); ?>)" class="w-9 h-9 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 flex items-center justify-center transition" title="View Details">
                                        <i data-lucide="eye" class="w-4 h-4"></i>
                                    </button>
                                    <button type="button" onclick="triggerProtectedAction(openEditInvoiceModal, <?php echo e($invoice->id); ?>)" class="w-9 h-9 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 flex items-center justify-center transition" title="Edit Invoice">
                                        <i data-lucide="edit" class="w-4 h-4"></i>
                                    </button>
                                    <button type="button" onclick="triggerProtectedAction(deleteInvoice, <?php echo e($invoice->id); ?>)" class="w-9 h-9 rounded-lg border border-red-100 bg-red-50 hover:bg-red-100 text-brand-red flex items-center justify-center transition" title="Delete Invoice">
                    ...
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <tr><td colspan="7" class="text-center py-10 text-slate-400">No matching invoices found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 px-6 py-4 no-print">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="text-sm text-slate-500">
                Showing <span class="font-bold text-navy"><?php echo e($invoices->total() === 0 ? 0 : $invoices->firstItem()); ?></span> to <span class="font-bold text-navy"><?php echo e($invoices->total() === 0 ? 0 : $invoices->lastItem()); ?></span> of <span class="font-bold text-navy"><?php echo e($invoices->total()); ?></span> invoices
            </div>
            <div class="flex items-center gap-1.5">
                ...
                <?php if($invoices->onFirstPage()): ?>
                    <span class="<?php echo e($disabledClass); ?>"><i data-lucide="chevron-left" class="w-4 h-4"></i></span>
                <?php else: ?>
                    <a href="<?php echo e($invoices->previousPageUrl()); ?>" class="<?php echo e($btnClass); ?>"><i data-lucide="chevron-left" class="w-4 h-4"></i></a>
                <?php endif; ?>
                <?php for($page = 1; $page <= $invoices->lastPage(); $page++): ?>
                    <?php if($page === $invoices->currentPage()): ?>
                        <span class="<?php echo e($activeClass); ?>"><?php echo e($page); ?></span>
                    <?php else: ?>
                        <a href="<?php echo e($invoices->url($page)); ?>" class="<?php echo e($btnClass); ?>"><?php echo e($page); ?></a>
                    <?php endif; ?>
                <?php endfor; ?>
                <?php if($invoices->hasMorePages()): ?>
                    <a href="<?php echo e($invoices->nextPageUrl()); ?>" class="<?php echo e($btnClass); ?>"><i data-lucide="chevron-right" class="w-4 h-4"></i></a>
                <?php else: ?>
                    <span class="<?php echo e($disabledClass); ?>"><i data-lucide="chevron-right" class="w-4 h-4"></i></span>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
<script>
window.canManageAR = <?php echo json_encode(\App\Models\Role::activeRoleCanManageAR(), 15, 512) ?>;

const invoicesData = <?php echo json_encode($invoices->items(), 15, 512) ?>;
...
function handleProtectedNavigation(targetUrl) {
    if (!window.canManageAR) { showAccessDeniedModal(); } 
    else { window.location.href = targetUrl; }
}

function triggerProtectedAction(callbackFunction, ...args) {
    if (!window.canManageAR) { showAccessDeniedModal(); } 
    else { callbackFunction(...args); }
}
    ...
    invoice.items.forEach(item => {
        itemsRows += `
            <tr class="border-b border-slate-100 text-sm">
                ...
                <td class="px-4 py-3 text-right font-semibold text-slate-700">${formatCurrency(item.quantity * item.unit_price)}</td>
            </tr>`;
    });
        ...
        <div class="space-y-6">
            ...
            <div class="flex justify-end">
                <div class="w-full sm:w-72 bg-slate-50 border border-slate-100 rounded-xl p-4 text-sm space-y-2">
                    <div class="flex justify-between text-slate-500"><span>Subtotal</span><span>${formatCurrency(invoice.subtotal)}</span></div>
                    <div class="flex justify-between text-slate-500"><span>Tax (12%)</span><span>${formatCurrency(invoice.tax)}</span></div>
                    <hr class="border-slate-200">
                    <div class="flex justify-between text-base font-bold text-navy"><span>Total</span><span>${formatCurrency(invoice.total)}</span></div>
                    <div class="flex justify-between text-sm font-bold text-brand-red"><span>Balance</span><span>${formatCurrency(invoice.balance)}</span></div>
                    <div class="pt-2 text-center"><span class="${badgeClass} px-4 py-1.5 rounded-full text-xs">${invoice.status}</span></div>
                </div>
            </div>
            <div class="flex justify-end pt-2">
                <button type="button" onclick="AppUI.closeModal()" class="rounded-xl px-5 py-2.5 text-sm font-semibold text-slate-600 border border-slate-200 hover:bg-slate-50 transition">Close</button>
            </div>
        </div>
    `, 'lg');
}
        ...
        itemsFormHtml += `
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 border-b border-slate-100 pb-3 items-center">
                <div>
                    ...
                    <input class="edit-description w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-navy focus:border-navy" value="${item.description}">
                </div>
                <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase mb-1 text-center">Qty</label>
                    <input type="number" class="edit-quantity text-center w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-navy focus:border-navy" value="${item.quantity}" oninput="calculateEditTotal()">
                </div>
                <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase mb-1 text-right">Unit Price (₱)</label>
                    <input type="number" class="edit-price text-right w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-navy focus:border-navy" value="${item.unit_price}" oninput="calculateEditTotal()">
                </div>
            </div>`;
    });
    ...
    AppUI.openModal(`
        <div class="space-y-6">
            ...
            <form id="editInvoiceForm" class="space-y-4" onsubmit="event.preventDefault(); saveInvoiceEdit(${invoice.id});">
                ...
                <div id="editItemsContainer" class="space-y-3 max-h-52 overflow-y-auto pr-1">${itemsFormHtml}</div>
                <div class="flex justify-end pt-3">
                    <div class="w-72 border border-slate-100 rounded-xl bg-slate-50 p-4 text-xs space-y-2">
                        <div class="flex justify-between text-slate-500"><span>Subtotal</span><span id="editSubtotal">${formatCurrency(invoice.subtotal)}</span></div>
                        <div class="flex justify-between text-slate-500"><span>Tax (12%)</span><span id="editTax">${formatCurrency(invoice.tax)}</span></div>
                        <hr class="border-slate-200">
                        <div class="flex justify-between text-sm font-bold text-navy"><span>New Total</span><span id="editTotal">${formatCurrency(invoice.total)}</span></div>
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                    <button type="button" onclick="AppUI.closeModal()" class="rounded-xl px-5 py-2.5 text-sm font-semibold text-slate-600 border border-slate-200 hover:bg-slate-50 transition">Cancel</button>
                    <button type="submit" class="rounded-xl px-5 py-2.5 text-sm font-semibold text-white bg-navy hover:bg-navy-700 transition shadow-sm">Save Changes</button>
                </div>
            </form>
        </div>
    `, 'lg');
...
function calculateEditTotal() {
    ...
    let subtotal = 0;
    quantities.forEach((qty, index) => { subtotal += Number(qty.value) * Number(prices[index].value); });
    ...
    document.getElementById('editTotal').innerText = formatCurrency(subtotal + tax);
}
...
function saveInvoiceEdit(id) {
    ...
    .then(data => {
        AppUI.closeModal();
        if (data.success) {
            if (typeof AppUI.showToast === 'function') { AppUI.showToast(data.message || 'Invoice updated successfully.', 'success'); }
            window.location.reload();
        } else {
            if (typeof AppUI.showToast === 'function') { AppUI.showToast('Something went wrong.', 'error'); }
        }
    })
    .catch(() => {
        AppUI.closeModal();
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Finance-Accounting\resources\views/accounts-receivable/allinvoices.blade.php ENDPATH**/ ?>