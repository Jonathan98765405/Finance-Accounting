...
<?php $__env->startSection('page-title-heading', 'Financial Reports & Compliance'); ?>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-6">
                <div class="relative w-44 h-44 shrink-0">
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        <p id="compliance-total-audits" class="text-2xl font-extrabold text-navy"><?php echo e($complianceDonut['total']); ?></p>
                    </div>
                </div>
                <div class="space-y-3 text-sm w-full">
                    <div class="flex items-center justify-between">
                        ...
                        <span id="compliance-complaint-pct" class="font-semibold"><?php echo e($complianceDonut['complaint']); ?>%</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="flex items-center gap-2"><i
                                class="w-2.5 h-2.5 rounded-full bg-navy-600 inline-block"></i> Pending</span>
                        <span id="compliance-pending-pct" class="font-semibold"><?php echo e($complianceDonut['pending']); ?>%</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="flex items-center gap-2"><i
                                class="w-2.5 h-2.5 rounded-full bg-brand-orange inline-block"></i> Failed</span>
                        <span id="compliance-failed-pct" class="font-semibold"><?php echo e($complianceDonut['failed']); ?>%</span>
                    </div>
...
<?php $__env->startPush('scripts'); ?>
    <script>
        ...
        const CURRENT_YEAR = <?php echo e($selectedYear); ?>;
        const ALL_YEARS = <?php echo json_encode($years, 15, 512) ?>;
        const yearOptionsHtml = (selected) => ALL_YEARS.map(y => `<option value="${y}" ${String(y) === String(selected) ? 'selected' : ''}>${y}</option>`).join('');

        const REVENUE_BY_YEAR = { [CURRENT_YEAR]: <?php echo json_encode($revenueSeries, 15, 512) ?> };
        const PROFIT_BY_YEAR = { [CURRENT_YEAR]: <?php echo json_encode($profitSeries, 15, 512) ?> };
        const REPORTS = { [CURRENT_YEAR]: <?php echo json_encode($monthlyReports, 15, 512) ?> };
        const COMPLIANCE_DONUT = { [CURRENT_YEAR]: <?php echo json_encode($complianceDonut, 15, 512) ?> };
        let AUDITS = <?php echo json_encode($audits, 15, 512) ?>;
        ...
        const ACTIVITIES = <?php echo json_encode($activities, 15, 512) ?>;

        async function loadOverviewYear(year) {
            if (String(year) === String(CURRENT_YEAR)) return; 

            const res = await fetch(`<?php echo e(url('/financial-reports/overview/data')); ?>?year=${year}`, {
        ...
        function openAuditEditModal(id) {
            ...
            document.getElementById('audit-edit-save').addEventListener('click', function () {
                ...
                fetch(`<?php echo e(url('/financial-reports/audits')); ?>/${a.id}`, {
                    method: 'PUT',
                    headers: {
        ...
        function deleteAudit(id) {
            ...
            fetch(`<?php echo e(url('/financial-reports/audits')); ?>/${id}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': csrfToken ?? '',
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Finance-Accounting\resources\views/financial-reports/overview.blade.php ENDPATH**/ ?>