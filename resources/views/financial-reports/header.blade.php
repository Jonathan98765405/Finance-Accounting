...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-6 bg-white rounded-2xl shadow-card p-6">
    ...
    @foreach ($stats as $i => $stat)
        <div class="flex items-start gap-4 {{ $i > 0 ? 'sm:border-l sm:border-slate-100 sm:pl-6' : '' }}">
            <div
                class="flex h-14 w-14 shrink-0 items-center justify-center rounded-full {{ $stat['iconBg'] }} text-white shadow-md">
            ...
            <div>
                ...
                <p class="text-2xl font-extrabold text-navy mt-1">{{ $stat['value'] }}</p>

                @if (!empty($stat['badge']))
                    <span
                        class="inline-block mt-1.5 rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-semibold {{ $stat['trendColor'] }}">
                        {{ $stat['trend'] }}
                    </span>
                @else
                    <p class="flex items-center gap-1 text-xs font-semibold {{ $stat['trendColor'] }} mt-1">
                        @if (!empty($stat['trendIcon']))
                            <i data-lucide="{{ $stat['trendIcon'] }}" class="w-3.5 h-3.5"></i>
                        @endif
                        {{ $stat['trend'] }}
                    </p>
                @endif
      ...
      `, 'lg');
          ...
          `;
                ...
                document.getElementById('add-audit-form').addEventListener('submit', function (e) {
                    ...
                    fetch('{{ route('financial-reports.audits.store') }}', {
                        method: 'POST',
                        headers: {
