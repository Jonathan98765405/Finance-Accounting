    ...
    <div class="relative overflow-hidden rounded-2xl bg-navy text-white p-6 sm:p-8 mb-6 shadow-card">
        <div class="relative z-10 max-w-2xl">
            <h3 id="welcome-role" class="text-2xl sm:text-3xl font-extrabold mb-2">
                Welcome Back, {{ strtoupper(session('active_role_label', 'Administrator')) }}!
            </h3>
            <p class="text-slate-200 text-sm sm:text-base leading-relaxed">
                Here's a snapshot of our company's financial health for {{ $currentQuarter ?? 'Q2 · FY 2026' }}.
        ...
        @foreach ($stats as $stat)
            <div class="bg-white rounded-2xl shadow-card p-5 sm:p-6">
                <div class="flex items-start justify-between mb-4">
                    <div
                        class="flex h-11 w-11 items-center justify-center rounded-full {{ $stat['iconBg'] }} text-white shadow-md">
        ...
        @foreach ($modules as $mod)
            @if ($mod['href'])
                <a href="{{ $mod['href'] }}"
                    ...
                    <div class="flex items-start justify-between mb-5">
                        <div
                            class="flex h-14 w-14 items-center justify-center rounded-xl {{ $mod['iconBg'] }} {{ $mod['iconColor'] }}">
            ...
            @else
                <div
                    ...
                    <p class="text-slate-400 text-xs sm:text-sm mt-0.5">{{ $mod['subtitle'] }}</p>
                    <p class="text-lg sm:text-xl font-extrabold text-navy mt-4">{{ $mod['value'] }}</p>
                    <p class="text-slate-400 text-xs mt-0.5 uppercase tracking-wide">{{ $mod['footnote'] }} · route not set up</p>
                </div>
            @endif
        @endforeach
    </div>
