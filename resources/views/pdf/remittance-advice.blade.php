...
<body>
    ...
    <div class="muted" style="margin-bottom:16px;">Reference No. {{ $payment->remittance_number }}</div>

    <table class="info">
        <tr>
            <td width="50%">
                <strong>Paid To:</strong><br>
                {{ $payment->invoice->supplier->name }}<br>
                {{ $payment->invoice->supplier->address }}<br>
                {{ $payment->invoice->supplier->email }}
            </td>
            <td width="50%">
                <strong>Payment Date:</strong> {{ $payment->payment_date?->format('M d, Y') }}<br>
    ...
    <table class="line-items">
        ...
        <tbody>
            <tr>
                ...
                <td class="text-right">{{ $payment->invoice->currency }} {{ number_format((float) $payment->amount, 2) }}</td>
            </tr>
            <tr class="total-row">
