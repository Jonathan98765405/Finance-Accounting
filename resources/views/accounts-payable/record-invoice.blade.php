@extends('layouts.app')
@section('page-title', 'Finance & Accounting | Record Supplier Invoice')
@section('page-title-heading', 'Record Incoming Supplier Invoice')
@section('page-subtitle', 'Record supplier invoices before verification and payment processing.')
...
<div class="space-y-6">
    ...
    <form method="POST" action="{{ route('ap.record.store') }}" id="invoiceForm" class="space-y-6">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-semibold text-slate-500 uppercase tracking-wider flex justify-between items-center">
                        ...
                        <a href="{{ route('ap.po.create') }}" class="text-[11px] text-brand-blue hover:text-navy font-bold normal-case inline-flex items-center gap-0.5 transition">
                            <i data-lucide="plus" class="w-3.5 h-3.5"></i> New PO
                        </a>
                    </label>
                    <select class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 focus:border-brand-blue focus:outline-none focus:ring-1 focus:ring-brand-blue transition" 
                            name="po_number" id="poSelect">
                        <option value="" {{ old('po_number') ? '' : 'selected' }}>No PO / Not applicable</option>
                        @foreach ($purchaseOrders as $po)
                            <option value="{{ $po->po_number }}"
                                    data-supplier-id="{{ $po->supplier_id }}"
                                    {{ old('po_number') == $po->po_number ? 'selected' : '' }}>
            ...
            <div class="bg-white rounded-2xl shadow-card p-6 flex flex-col justify-between">
                <div>
                    ...
                    <textarea class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-800 focus:border-brand-blue focus:outline-none focus:ring-1 focus:ring-brand-blue transition"
                              name="remarks" 
                              rows="4" 
                              placeholder="Enter additional notes or remarks about this invoice...">{{ old('remarks') }}</textarea>
                </div>
            </div>
