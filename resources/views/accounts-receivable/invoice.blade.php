...
<div class="w-full">
    ...
    <form id="invoiceForm" method="POST" action="{{ route('receivable.invoice.store') }}" onsubmit="return submitInvoice(event)">
        ...
        <!-- Hidden fields carrying computed totals, submitted alongside the form -->
        ...
        <!-- Header Actions (Aligned Top Right) -->
        <div class="flex items-center justify-end mb-6">
            <div class="flex gap-3">
                <a href="{{ route('receivable.dashboard') }}"
                   class="px-6 py-2.5 rounded-xl border border-slate-300 bg-white text-slate-700 font-medium hover:bg-slate-50 transition shadow-sm">
                    Cancel
                </a>
                <button type="submit"
                        class="px-6 py-2.5 rounded-xl bg-navy text-white font-medium hover:bg-navy-700 transition shadow-card flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-card p-7 border border-slate-100">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-1">Auto-filled from the selected customer's record.</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-card p-7 border border-slate-100">
                <h2 class="text-lg font-bold text-navy mb-6 border-b border-slate-100 pb-4 flex items-center gap-2">
                    <i data-lucide="file-text" class="w-5 h-5 text-brand-orange"></i>
                    Invoice Information
                </h2>
                ...
                <div class="grid grid-cols-2 gap-5">
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-1">Preview only — the final number is generated when saved.</p>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mt-6">
            ...
            <div class="lg:col-span-3 bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden">
                <div class="flex items-center justify-between p-6 border-b border-slate-100">
                    <h2 class="text-lg font-bold text-navy flex items-center gap-2">
                        <i data-lucide="list" class="w-5 h-5 text-brand-blue"></i>
                        Invoice Items
                    </h2>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full" id="itemsTable">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr>
                                <td class="px-6 py-4">
                                    <input class="description w-full border border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:border-brand-blue focus:ring-1 focus:ring-brand-blue transition text-sm" type="text" name="description[]" placeholder="Enter Item Description">
                                </td>
                                <td class="px-4 py-4">
                                    <input type="number" name="quantity[]" value="1" min="1" class="qty w-full border border-slate-200 rounded-xl px-3 py-2.5 text-center outline-none focus:border-brand-blue focus:ring-1 focus:ring-brand-blue transition text-sm">
                                </td>
                                <td class="px-4 py-4">
                                    <input type="number" name="unit_price[]" step="0.01" value="0" class="price w-full border border-slate-200 rounded-xl px-3 py-2.5 text-right outline-none focus:border-brand-blue focus:ring-1 focus:ring-brand-blue transition text-sm">
                                </td>
                                <td class="px-4 py-4">
                                    <input type="text" readonly class="amount w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-right text-slate-500 cursor-not-allowed outline-none text-sm" value="₱0.00">
                                </td>
            ...
            <div class="bg-white rounded-2xl shadow-card border border-slate-100 h-fit">
                <div class="p-6 border-b border-slate-100">
                    <h2 class="text-lg font-bold text-navy flex items-center gap-2">
                        <i data-lucide="calculator" class="w-5 h-5 text-brand-orange"></i>
                        Summary
                    </h2>
                </div>
                <div class="p-6 space-y-4">
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="font-semibold text-slate-700" id="subtotal">₱0.00</span>
                    </div>
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-500">Tax (12%)</span>
                        <span class="font-semibold text-slate-700" id="tax">₱0.00</span>
                    </div>
                    ...
                    <div class="flex justify-between items-center pt-2">
                        ...
                        <span class="text-xl font-bold text-brand-greenDark" id="total">₱0.00</span>
                    </div>
                </div>
            </div>
...
<script>
    // Full customer records (id, name, address, email, phone) sourced from
    // the customers table via AccountsReceivableController@invoice.
    ...
    // Client-side preview only. The real invoice_number is generated
    // server-side in AccountsReceivableController@storeInvoice.
    ...
    function calculateTotals() {
        ...
        // Keep the hidden numeric fields in sync — these are what actually get submitted.
    ...
    function addRow(){
        ...
        // Re-initialize Lucide icons for the new row's trash button
        if (typeof lucide !== 'undefined') {
            lucide.createIcons();
        }
    }
    ...
    // Client-side sanity checks before letting the real POST go through
    // to AccountsReceivableController@storeInvoice.
