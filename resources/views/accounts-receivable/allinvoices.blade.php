...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6">
            ...
            <h2 class="text-3xl sm:text-4xl font-extrabold text-navy mt-2">{{ $totalInvoices }}</h2>
        </div>
        <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6">
            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Paid</p>
            <h2 class="text-3xl sm:text-4xl font-extrabold text-brand-green mt-2">{{ $paidInvoices }}</h2>
        </div>
        <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6">
            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Unpaid</p>
            <h2 class="text-3xl sm:text-4xl font-extrabold text-brand-orange mt-2">{{ $unpaidInvoices }}</h2>
        </div>
        <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 p-6">
            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Overdue</p>
            <h2 class="text-3xl sm:text-4xl font-extrabold text-brand-red mt-2">{{ $overdueInvoices }}</h2>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white rounded-2xl shadow-card border border-slate-200/80 p-4 no-print">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="javascript:void(0)" onclick="handleProtectedNavigation('{{ route('receivable.invoice') }}')"
        ...
        <form id="filterForm" method="GET" action="{{ route('receivable.allinvoices') }}">
            <div class="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" id="searchInput" value="{{ request('search') }}" placeholder="Search invoice number or customer..." class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy transition">
                </div>
                <div class="flex flex-wrap items-center gap-3">
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($invoices as $invoice)
                        <tr class="hover:bg-slate-50/70 transition">
                            <td class="px-6 py-4 font-bold text-navy">{{ $invoice->invoice_number }}</td>
                            <td class="px-6 py-4 text-slate-600 font-medium">{{ $invoice->customer->customer_name ?? 'No Customer' }}</td>
                            <td class="px-6 py-4 text-slate-500">{{ \Carbon\Carbon::parse($invoice->invoice_date)->format('M d, Y') }}</td>
                            <td class="px-6 py-4 text-slate-500">{{ \Carbon\Carbon::parse($invoice->due_date)->format('M d, Y') }}</td>
                            <td class="px-6 py-4 text-right font-bold text-slate-700">&#8369;{{ number_format($invoice->total, 2) }}</td>
                            <td class="px-6 py-4 text-center">
                                ...
                                @endphp
                                <span class="{{ $badgeClass }} px-3 py-1 rounded-full text-xs">{{ $invoice->status }}</span>
                            </td>
                            <td class="px-6 py-4 text-center no-print">
                                <div class="flex justify-center gap-1.5">
                                    <button type="button" onclick="openInvoiceDetailsModal({{ $invoice->id }})" class="w-9 h-9 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 flex items-center justify-center transition" title="View Details">
                                        <i data-lucide="eye" class="w-4 h-4"></i>
                                    </button>
                                    <button type="button" onclick="triggerProtectedAction(openEditInvoiceModal, {{ $invoice->id }})" class="w-9 h-9 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 flex items-center justify-center transition" title="Edit Invoice">
                                        <i data-lucide="edit" class="w-4 h-4"></i>
                                    </button>
                                    <button type="button" onclick="triggerProtectedAction(deleteInvoice, {{ $invoice->id }})" class="w-9 h-9 rounded-lg border border-red-100 bg-red-50 hover:bg-red-100 text-brand-red flex items-center justify-center transition" title="Delete Invoice">
    ...
    <div class="bg-white rounded-2xl shadow-card border border-slate-200/80 px-6 py-4 no-print">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="text-sm text-slate-500">
                Showing <span class="font-bold text-navy">{{ $invoices->total() === 0 ? 0 : $invoices->firstItem() }}</span> to <span class="font-bold text-navy">{{ $invoices->total() === 0 ? 0 : $invoices->lastItem() }}</span> of <span class="font-bold text-navy">{{ $invoices->total() }}</span> invoices
            </div>
