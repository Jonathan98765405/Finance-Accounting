...
@section('content')
    ...
    @php
        // $items comes from the controller: old('items', ...) on validation failure,
        // otherwise the invoice's existing line items.
    @endphp
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">
                ...
                <span class="mx-1">&gt;</span> {{ $invoice->invoice_no }}
            </p>
        </div>
    ...
    <form id="invoice-form" method="POST" action="{{ route('sales-invoices.update', $invoice) }}">
        @csrf
        @method('PUT')
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="billing_address" name="billing_address" rows="2" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500">{{ old('billing_address', $invoice->billing_address) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input id="billing_email" name="billing_email" type="email" value="{{ old('billing_email', $invoice->billing_email) }}" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500">
                        ...
                        <div>
                            ...
                            <input id="billing_phone" name="billing_phone" type="text" value="{{ old('billing_phone', $invoice->billing_phone) }}" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" value="{{ $invoice->invoice_no }}" readonly class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5" for="invoice_date">Invoice Date</label>
                        <input id="invoice_date" name="invoice_date" type="date" value="{{ old('invoice_date', $invoice->invoice_date->format('Y-m-d')) }}" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500">
                    ...
                    <div>
                        ...
                        <input id="due_date" name="due_date" type="date" value="{{ old('due_date', optional($invoice->due_date)->format('Y-m-d')) }}" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500">
                    ...
                    <div>
                        ...
                        <select id="payment_terms" name="payment_terms" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500">
                            ...
                            @foreach ($terms as $label => $days)
                                <option value="{{ $label }}" data-days="{{ $days }}" @selected(old('payment_terms', $invoice->payment_terms) === $label)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-2">
                        ...
                        <select id="payment_status" name="payment_status" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500">
                            <option value="Unpaid" @selected(old('payment_status', $invoice->payment_status) === 'Unpaid')>Unpaid</option>
                            <option value="Paid" @selected(old('payment_status', $invoice->payment_status) === 'Paid')>Paid</option>
                        </select>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-5 h-fit">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <input type="number" id="discount_amount" name="discount_amount" value="{{ old('discount_amount', $invoice->discount_amount) }}" min="0" step="0.01" class="w-28 text-right rounded-lg border border-slate-200 px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500">
                    </div>
